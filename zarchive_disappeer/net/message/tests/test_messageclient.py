"""
test_messageclient.py

Test suite for the MessageClient class object, client to send new message to remote peer
...
class TestImports(unittest.TestCase):
    ...
    def test_abstract_client(self):
        self.assertEqual(abstractclient, messageclient.abstractclient)

    def test_tempfile(self):
        self.assertEqual(tempfile, messageclient.tempfile)

    def test_ssl(self):
        self.assertEqual(ssl, messageclient.ssl)

    def test_contactprotocol(self):
        self.assertEqual(contactprotocol, messageclient.contactprotocol)

    def test_constants(self):
        self.assertEqual(constants, messageclient.constants)
...
class TestClassBasics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.argnamespace.command = 'MSG'
        self.argnamespace.host = 'host'
        self.argnamespace.port = 'port'
        self.argnamespace.nonce = 'nonce'
        self.argnamespace.ssl_key = 'ssl_key'
        self.argnamespace.queue = MagicMock()
        self.x = messageclient.MessageClient(self.argnamespace)

    def test_instance(self):
        self.assertIsInstance(self.x, messageclient.MessageClient)
    ...
    def test_build_error_dict_returns_correct_dict(self):
        self.x.error = 'error'
        desc = constants.command_list.Send_New_Message_Client_Err
        target = dict(desc=desc,
                      error=self.x.error,
                      host=self.x.host,
                      port=self.x.port,
                      nonce=self.x.nonce,
                      argnamespace=self.x.argnamespace)
    ...
    def test_build_result_dict_returns_correct_dict_valid_nonce(self):
        ...
        target = dict(desc=constants.command_list.Send_New_Message_Client_Res,
                      result=input_dict,
                      nonce=self.x.nonce,
                      nonce_valid=True,
                      host=self.x.host,
