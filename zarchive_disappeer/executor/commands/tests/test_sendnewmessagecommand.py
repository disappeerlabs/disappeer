"""
test_sendnewmessagecommand.py

Test suite for SendNewMessageCommand module and class object
...
from disappeer.executor.commands import sendnewmessagecommand as command


class TestSendNewMessageCommand(unittest.TestCase):

    def setUp(self):
        self.receiver = MagicMock()
        self.payload = dict()
        self.kwargs = dict(payload=self.payload)
        self.x = command.SendNewMessageCommand(self.receiver, **self.kwargs)

    def test_instance(self):
        self.assertIsInstance(self.x, command.SendNewMessageCommand)
        self.assertIsInstance(self.x, abstractcommand.AbstractCommand)

    def test_name_attr(self):
        self.assertEqual(self.x.name, constants.command_list.Send_New_Message)
